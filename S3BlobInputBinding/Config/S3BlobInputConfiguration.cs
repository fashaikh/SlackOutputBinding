...
{
    /// <summary>
    /// Extension for binding <see cref="S3BlobInputMessage"/>.
    /// </summary>
    public class S3BlobInputConfiguration : IExtensionConfigProvider
    {
    ...
    {
        // add converter between JObject and S3BlobInputMessage
        // Allows a user to bind to IAsyncCollector<JObject>, and the sdk will convert that to IAsyncCollector<S3BlobInputMessage>
        context.AddConverter<JObject, S3BlobInputMessage>(input => input.ToObject<S3BlobInputMessage>());

        // Add a binding rule for Collector
        context.AddBindingRule<S3BlobInputAttribute>()
            .BindToCollector<S3BlobInputMessage>(attr => new S3BlobInputAsyncCollector(this, attr));
    }
    }
}
